...
const createCourseInfo = (course) => {
    ...
    teacherName.appendChild(document.createTextNode(`Teacher: ${course.courseTeacher}`));
    teacherName.classList.add("teacher-name");
    const description = document.createElement("p");
    description.appendChild(document.createTextNode(`${course.courseDescription}`));
    ...
    location.appendChild(document.createTextNode(`Location: ${course.courseLocation}`));
...
const readMoreClickHandler = (readButtons) => {
...
const adminReadMoreClickHandler = (readButtons) => {
    readButtons.forEach((readbutton) => {
        const src = readbutton.getAttribute("src");
        const courseId = readbutton.getAttribute("id");
        readbutton.addEventListener("click", () => {
...
const createDateInfo = (course) => {
    const firstDate = document.createElement("p");
    firstDate.classList.add("dates");
    firstDate.appendChild(document.createTextNode(`The first course start at ${course.dateOne}`));
    const secoundDate = document.createElement("p");
    secoundDate.classList.add("dates");
    secoundDate.appendChild(document.createTextNode(`The secound course start at ${course.dateTwo}`));
    const thirdDate = document.createElement("p");
    thirdDate.classList.add("dates");
    thirdDate.appendChild(document.createTextNode(`The third start at ${course.dateThree}`));
    return [firstDate, secoundDate, thirdDate];
};
const createCompleteCourseCard = (course) => {
...
const createClearLocalStorageButton = (className, text) => {
...
export { createCard, readMoreClickHandler, adminReadMoreClickHandler, createCompleteCourseCard, createClearLocalStorageButton, };
